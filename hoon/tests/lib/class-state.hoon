...
|%
++  text
  'compose\0a  class Point(x:@ y:@) {\0a    inc(q:@) -> @ {\0a      +(q)\0a    }\0a  }\0a;\0a\0alet point-1 = Point(70 80);\0alet point-2 = Point(90 100);\0a((point-2.x() point-2.y()) (point-1.x() point-1.y()))\0a\0a/*\0a!=\0a=>  mini=mini\0a=>\0a  ^=  door\0a  |_  [x=@ y=@]\0a  ++  add\0a    |=  p=[x=@ y=@]\0a    [(add:mini x x.p) (add:mini y y.p)]\0a  ++  inc\0a    |=  q=@\0a    +(q)\0a  --\0a=/  point-1\0a  ~(. door [70 80])\0a=/  point-2\0a  ~(. door [90 100])\0a[[+13 +12]:point-2 [+13 +12]:point-1]\0a*/\0a'
++  test-tokenize
  %+  expect-eq:test
...
++  test-jeam
  %+  expect-eq:test
    !>  ^-  jock:jock
        [%compose p=[%class state=[p=[%state p=[[p=[p=[%atom p=%number q=%.n] name='x'] q=[p=[%atom p=%number q=%.n] name='y']] name='']] name='Point'] arms=[n=[p=%inc q=[%method type=[p=[%core p=[%.y p=[inp=[~ [p=[%atom p=%number q=%.n] name='q']] out=[p=[%atom p=%number q=%.n] name='']]] q=~] name='inc'] body=[%lambda p=[arg=[inp=[~ [p=[%atom p=%number q=%.n] name='q']] out=[p=[%atom p=%number q=%.n] name='']] body=[%increment val=[%limb p=~[[%name p=%q]]]] context=~]]]] l=~ r=~]] q=[%let type=[p=[%none p=~] name='point-1'] val=[%call func=[%limb p=~[[%type p='Point']]] arg=[~ [p=[%atom p=[[%number p=70] q=%.n]] q=[%atom p=[[%number p=80] q=%.n]]]]] next=[%let type=[p=[%none p=~] name='point-2'] val=[%call func=[%limb p=~[[%type p='Point']]] arg=[~ [p=[%atom p=[[%number p=90] q=%.n]] q=[%atom p=[[%number p=100] q=%.n]]]]] next=[p=[p=[%call func=[%limb p=~[[%name p=%point-2] [%name p=%x]]] arg=~] q=[%call func=[%limb p=~[[%name p=%point-2] [%name p=%y]]] arg=~]] q=[p=[%call func=[%limb p=~[[%name p=%point-1] [%name p=%x]]] arg=~] q=[%call func=[%limb p=~[[%name p=%point-1] [%name p=%y]]] arg=~]]]]]]
    !>  (jeam:jock text)
::
++  test-mint
  %+  expect-eq:test
