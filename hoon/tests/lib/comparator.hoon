...
++  test-jeam
  %+  expect-eq:test
    !>  ^-  jock:jock
        [%let type=[p=[%none ~] name=%a] val=[%atom p=[[%loobean p=%.y] q=%.n]] next=[%let type=[p=[%none ~] name=%b] val=[%compare comp=%'==' a=[%limb p=~[[%name p=%a]]] b=[%atom p=[[%loobean p=%.y] q=%.n]]] next=[%let type=[p=[%none ~] name=%c] val=[%compare comp=%'<' a=[%limb p=~[[%name p=%a]]] b=[%atom p=[[%number p=1] q=%.n]]] next=[%let type=[p=[%none ~] name=%d] val=[%compare comp=%'>' a=[%limb p=~[[%name p=%a]]] b=[%atom p=[[%number p=2] q=%.n]]] next=[%let type=[p=[%none ~] name=%e] val=[%compare comp=%'!=' a=[%limb p=~[[%name p=%b]]] b=[%atom p=[[%loobean p=%.y] q=%.n]]] next=[%let type=[p=[%none ~] name=%f] val=[%compare comp=%'<=' a=[%limb p=~[[%name p=%a]]] b=[%atom p=[[%number p=1] q=%.n]]] next=[%let type=[p=[%none ~] name=%g] val=[%compare comp=%'>=' a=[%limb p=~[[%name p=%a]]] b=[%atom p=[[%number p=2] q=%.n]]] next=[%limb p=~[[%name p=%g]]]]]]]]]]
    !>  (jeam:jock text)
::
++  test-mint
  %+  expect-eq:test
...
++  test-nock
  %+  expect-eq:test
    :: !>  .*(0 [8 [1 0] 8 [5 [0 2] 1 0] 8 [11 6.845.548 0 0] 8 [11 6.845.543 0 0] 8 [6 [5 [0 14] 1 0] [1 1] 1 0] 8 [11 6.648.940 0 0] 8 [11 6.648.935 0 0] 0 2])
    !>  ~
    :: !>  .*(0 (mint:jock text))
    !>  ~
::
