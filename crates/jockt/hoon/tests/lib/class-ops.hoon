...
|%
++  text
  'compose\0a  class Point(x:@ y:@) {\0a   add(p:(x:@ y:@)) -> Point {\0a     (x + p.x\0a      y + p.y)\0a   }\0a  }\0a;\0a\0alet point-1 = Point(14 104);\0apoint-1 = point-1.add(28 38);\0a(point-1.x() point-1.y())\0a\0a/*\0a!=\0a=>  mini=mini\0a=>\0a  ^=  door\0a  |_  [x=@ y=@]\0a  ++  add\0a    |=  p=[x=@ y=@]\0a    [(add:mini x x.p) (add:mini y y.p)]\0a  --\0a=/  point-1\0a  ~(. door [14 104])\0a=.  point-1  ~(. door (add:point-1 [28 38]))\0a[+12 +13]:point-1\0a\0a!=\0a=>  mini=mini\0a=>\0a  ^=  door\0a  |_  [x=@ y=@]\0a  ++  add\0a    |=  p=[x=@ y=@]\0a    [(add:mini x x.p) (add:mini y y.p)]\0a  --\0a~(. door [14 104])\0a*/\0a'
++  test-tokenize
  %+  expect-eq:test
...
++  test-jeam
  %+  expect-eq:test
    !>  ^-  jock:jock
        [%compose p=[%class state=[p=[%state p=[[p=[p=[%atom p=%number q=%.n] name='x'] q=[p=[%atom p=%number q=%.n] name='y']] name='']] name='Point'] arms=[n=[p=%add q=[%method type=[p=[%core p=[%.y p=[inp=[~ [[p=[p=[%atom p=%number q=%.n] name='x'] q=[p=[%atom p=%number q=%.n] name='y']] name='p']] out=[p=[%limb p=~[[%type p='Point']]] name='Point']]] q=~] name='add'] body=[%lambda p=[arg=[inp=[~ [[p=[p=[%atom p=%number q=%.n] name='x'] q=[p=[%atom p=%number q=%.n] name='y']] name='p']] out=[p=[%limb p=~[[%type p='Point']]] name='Point']] body=[p=[%operator op=%'+' a=[%limb p=~[[%name p=%x]]] b=[~ [%limb p=~[[%name p=%p] [%name p=%x]]]]] q=[%operator op=%'+' a=[%limb p=~[[%name p=%y]]] b=[~ [%limb p=~[[%name p=%p] [%name p=%y]]]]]] context=~]]]] l=~ r=~]] q=[%let type=[p=[%none p=~] name='point-1'] val=[%call func=[%limb p=~[[%type p='Point']]] arg=[~ [p=[%atom p=[[%number p=14] q=%.n]] q=[%atom p=[[%number p=104] q=%.n]]]]] next=[%edit limb=~[[%name p=%point-1]] val=[%call func=[%limb p=~[[%name p=%point-1] [%name p=%add]]] arg=[~ [p=[%atom p=[[%number p=28] q=%.n]] q=[%atom p=[[%number p=38] q=%.n]]]]] next=[p=[%call func=[%limb p=~[[%name p=%point-1] [%name p=%x]]] arg=~] q=[%call func=[%limb p=~[[%name p=%point-1] [%name p=%y]]] arg=~]]]]]
    !>  (jeam:jock text)
::
++  test-mint
  %+  expect-eq:test
